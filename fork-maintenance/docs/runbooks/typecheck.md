...
the `develop` checkout, which already contains the complete stack of case
commits. Imported upstream modules supply type context
...
but deliberately partial coverage: the rest of the case commits, dynamic
...
Create the separate tool environment, then check the checkout:
...
make -C fork-maintenance typecheck
```
...
The wrapper requires mypy 2.3.1, requires clean product paths before and after
checking so that it examines exactly the committed `HEAD`, validates that every
scope path is changed by a case commit, and reports the exact embedded source
and `HEAD` identities. It modifies no file in the checkout. A missing file or
...
fail with `attr-defined`. It does not modify the checkout or installed code.
Changing the function requires maintaining this non-vacuous control.
The ordinary offline `check` also exercises scope ownership, dirty product
paths, missing/unsafe source paths, input mutation and mypy failure
