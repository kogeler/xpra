...
At canonical master `dbc575b063abbc6df314a63ee530fdb218776327`,
`unit.client.x11_client_paint_test` is non-green in all three local matrix
legs. Baseline and no-backwards-compatibility fail `test_colors_png` and
`test_xterm_position_and_color`. The Cython-heavy leg errors in all four test
methods because the spawned server reports no valid encodings. Upstream commit
...
between matrix legs. Before applying this case after every upstream rebase,
run all three clean `quarantine*` gates. Each gate succeeds only when every
