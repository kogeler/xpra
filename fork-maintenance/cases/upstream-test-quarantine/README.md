...
upstream rebase, run all three clean `quarantine*` gates. Each gate succeeds
only when every listed module is still an exact ignored failure. If any module
passes, remove or narrow its quarantine entry and refresh this one case before
accepting the stack.
