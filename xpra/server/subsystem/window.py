...
class WindowServer(StubSubsystem):
    ...
    def _process_stacking(self, proto, packet: Packet) -> None:
        ...
        for ss in self.window_sources(exclude=source):
            if ss.window_sync_stacking:
                ss.send_window_stacking(stacking)
    ...
    def sync_focus(self, ss, wid: int) -> None:
        """ raise the window on the other clients that asked for focus synchronization """
        window = self._id_to_window.get(wid)
        if not window:
            return
        for s in self.window_sources(exclude=ss):
            if s.window_sync_focus:
